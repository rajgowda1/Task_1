...
import { clearCart, removeItem } from './Redux/Actions';
...
export default function Orders() {
  ...
  const [singleBuyId, setSingleBuyId] = useState(searchParams.get('id') || null)
  console.log(searchParams.get('id'));
  console.log(singleBuyId, "SingleBuy");
  ...
  useEffect(() => {
    ...
    if (singleBuyId) {
      setSingleProduct(myState.items.find((item) => singleBuyId === item._id))
    }
    secureGet(addUrl)
      .then((res) => {
        setAddData(res.data)
  ...
  const submit = () => {
    ...
    if (singleBuyId) {
      items = [{
        ...
        subTotal: singleProduct.subTotal
      }]
    }
    else {
      items = myState.items.map((item) => {
    ...
    securePost(createOrder, data)
      .then((res) => {
        ...
        if (singleBuyId) {
          dispatch(removeItem(singleProduct._id))
        } else {
          dispatch(clearCart())
        }
      })
              ...
              <div className='d-flex justify-content-center  ' >
                ...
                <div className='d-flex  justify-content-center rounded bg-white mt-1'>
                  ...
                  <div className='p-1   ' style={{ width: '140px', height: '140px' }}>
                    <Image className='' style={{ width: '140px', height: '140px' }} src={singleProduct?.images[0].url} />
                  </div>
                  <div className='m-2 p-3  ' style={{ width: '180px', height: '140px' }}>

                    <div >Product : {singleProduct?.name}</div>
                    <div> singleProduct Cost : ${singleProduct?.price}</div>
                    <div> Quantity : {singleProduct?.quantity}</div>
                  </div>

                  <div className='m-2 p-3 ' style={{ width: '180px', height: '140px' }}>

                    <div>Subtotal : ${singleProduct?.subTotal}</div>
